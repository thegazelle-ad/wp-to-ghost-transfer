...
exports.fromStream = function(stream) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    xml.on('endElement: pubDate', function(pd) {
      if (errorCheck) {
        console.error("Publish Date " + JSON.stringify(pd).slice(0, 50));
      }
    ...
    xml.on('endElement: wp:category', function(category) {
      if (errorCheck) {
        console.error("Category " + JSON.stringify(category).slice(0, 50));
      }
      var tag = {
    ...
    xml.on('endElement: wp:tag', function(category) {
      if (errorCheck) {
        console.error("Tag " + JSON.stringify(category).slice(0, 50));
      }
      var tag = {
        "id": parseInt(category['wp:term_id'], 10),
    ...
    xml.on('endElement: wp:wp_author', function(author) {
      if (errorCheck) {
        console.error("Author " + JSON.stringify(author).slice(0, 50));
      }
      var user = {
    ...
    xml.on('endElement: item', function(item) {
      if (errorCheck) {
        console.error("Item " + JSON.stringify(item).slice(0, 50));
      }
